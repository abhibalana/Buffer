...
class PlayerActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        dao= LikeSongDao()
        val song = pref.getTrack("Track","NO")
        initializeSong(song)

        sharesong.setOnClickListener {
            val sendIntent: Intent = Intent().apply {
                action = Intent.ACTION_SEND
                putExtra(Intent.EXTRA_TEXT,pref.read("SongUrl","No"))
                type = "text/plain"
            }
    ...
    fun playNext(){
        ...
        recent.addAll(rec)
        recent.addAll(constant.likeSong)
        val rnd = (0 until recent.size).random()
        val song = recent[rnd]
        initializeSong(song)
        if(!recent.contains(song)){
            recent.add(song)
            pref.writeArrayList("RecentList",recent)
        }
        viewModel =
    ...
    fun initializeSong(song:ItemsItem){
